...
import fr.liglab.adele.cilia.controller.impl.ChainControllerImpl;
import fr.liglab.adele.cilia.controller.impl.CreatorThread;
...
public class CiliaContextImpl implements CiliaContext {
	...
	public Chain addChain(Chain chain) {
		...
		synchronized (lockObject) {
			if (!chainInstances.containsKey(chain.getId())) {
				ChainController chainInstance = new ChainControllerImpl(bcontext, chain,
						creator);
	...
	public Chain getChain(String chainId) {
		...
		synchronized (lockObject) {
			if (chainInstances.containsKey(chainId)) {
				ChainController chaininstance = getChainInstance(chainId);
	 ...
	 * @return
	 */
	private ChainController getChainInstance(String chainId) {
		synchronized (lockObject) {
			ChainController chi = null;
			chi = (ChainController) chainInstances.get(chainId);
			return chi;
		}
	}
	...
	public void startChain(String chainId) {
		ChainController cinstance = null;
		synchronized (lockObject) {
			if (chainInstances.containsKey(chainId)) {
				cinstance = (ChainController) chainInstances.get(chainId);
			}
		}
		if (cinstance != null) {
			cinstance.start();
	...
	public void removeChain(String chainId) {
		ChainController toBeRemoved = null;
		Chain chain = null;
		if (chainId != null) {
			synchronized (lockObject) {
				if (chainInstances.containsKey(chainId)) {
					toBeRemoved = (ChainController) chainInstances.remove(chainId);
				}
			}
			if (toBeRemoved != null) {
	...
	public void stopChain(String chainId) {
		String msg;
		ChainController ci = null;
		if (chainId != null) {
			synchronized (lockObject) {
				if (chainInstances.containsKey(chainId)) {
					ci = (ChainController) chainInstances.get(chainId);
				}
			}
			if (ci != null) {
				ci.stop();
	...
	private void disposeChains() {
		...
		while (it != null && it.hasNext()) {
			String chid = (String) it.next();
			ChainController cc = getChainInstance(chid);
			cc.dispose();
		}
	}
