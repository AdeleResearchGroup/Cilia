...
import fr.liglab.adele.cilia.handler.dependency.ServiceUsage.Usage;
 ...
 */
public class Dependency extends DependencyModel implements FieldInterceptor,
		MethodInterceptor, ConstructorInjector {

	/* Evolutions */
	/* Call handler Adapter / mediator monitor-handler */
	...
	/**
	 * True, a second list of service is used 
	 * service pending injection
	 */
	private boolean m_useListInWait ;
	/**
	 * Cardinality max , -1 if no max 
	 */
	private int m_maxServiceInjected;
	...
	public Dependency(DependencyHandler handler, String field, Class spec, Filter filter,
			boolean isOptional, boolean isAggregate, boolean nullable, boolean isProxy,
			String identity, BundleContext context, int policy, Comparator cmp,
			String defaultImplem) {
		super(spec, isAggregate, isOptional, filter, cmp, policy, context, handler,
				handler.getInstanceManager());
	...
	public Dependency(DependencyHandler handler, String field, Class spec, Filter filter,
			boolean isOptional, boolean isAggregate, boolean nullable, boolean isProxy,
			String identity, BundleContext context, int policy, Comparator cmp,
			String defaultImplem, int maxCardinality, boolean immediate) {
		super(spec, isAggregate, isOptional, filter, cmp, policy, context, handler,
				handler.getInstanceManager());
		...
		if ((maxCardinality > 0)) {
			if (policy == DYNAMIC_PRIORITY_BINDING_POLICY) {
				m_useListInWait = true;
			}
			if ((policy == DYNAMIC_BINDING_POLICY)) {
				m_useListInWait = true;
			}
			if (m_useListInWait == true) {
				m_maxServiceInjected = maxCardinality;
			}
			else m_maxServiceInjected=-1 ;
		}
		m_isImmediate = immediate;
		if (!m_isImmediate) m_useListInWait=true ;
		
		if (logger.isDebugEnabled()) {
			logger.debug("Max service injected :" + m_maxServiceInjected);
			logger.debug("Immediate :" + m_isImmediate);
		}
	}
	...
	public synchronized boolean isServicesInWaitToInject() {
	 return m_useListInWait	 ;
	}
	
	public int getMaxServiceInjected() {
		return m_maxServiceInjected;
	}
	...
	protected void onObjectCreation(Object pojo) {
		...
		synchronized (this) {
			...
			// We are notified of an instance creation, we have to freeze when
			// the static policy is used
	 ...
	 */
	private void invokeCallback(DependencyCallback callback, ServiceReference ref,
			Object pojo) {
		try {
			if (pojo == null) {
			...
			m_handler.error("The method " + callback.getMethodName()
					...
					+ " throws an exception : " + e.getTargetException().getMessage(),
	...
	private Object createNullableObject() {
		...
		try {
			ClassLoader cl = new NullableClassLoader(getHandler().getInstanceManager()
					.getClazz().getClassLoader(), getSpecification().getClassLoader());

			m_nullable = Proxy.newProxyInstance(cl, new Class[] { getSpecification(),
					Nullable.class }, new NullableObject()); // NOPMD
	...
	public void start() {
		...
		if (isOptional() && !isAggregate()) {
			if (m_di == null) {
				...
				try {
					Class clazz = getHandler().getInstanceManager().getContext()
					...
					throw new IllegalStateException(
							"Cannot load the default-implementation (unexpected exception) "
									+ m_di + " : " + e.getMessage());
				}
			}
		}
			...
			if (isAggregate()) {
				...
				if (getSpecification().isInterface()) {
					...
					if (type == null || type.equals(DependencyHandler.SMART_PROXY)) {
						SmartProxyFactory proxyFactory = new SmartProxyFactory(this
					...
					m_handler
							.warn("Cannot create a proxy for a service dependency which is not an interface "
		...
		super.start();
		// Once the dependency is started, access to fields must be
		// synchronized.
		synchronized (this) {
			if (getBindingPolicy() == STATIC_BINDING_POLICY
					&& m_handler.getInstanceManager().getPojoObjects() != null) {
	 ...
	 * methods. If the dependency cache is not reset, the thread continues to
	 * get older services.
	 * 
	 * @param departs
			...
			if (usage.m_componentStack > 0) {
				...
				if (isAggregate()) {
					...
					if (ref != null) {
						...
						throw new RuntimeException("Service " + getSpecification()
								+ " unavailable");
					}
				}
			}
	...
	/**
	 * Creates the object to store in the given Thread Local. This object will
	 * be injected inside the POJO field.
	 * 
	 * @param usage
				...
				if (m_nullable == null && m_supportNullable) {
					m_handler
							.warn("["
									...
									+ m_field + " -> " + getSpecification().getName());
					createNullableObject();
				}
				usage.m_object = m_nullable; // Add null if the Nullable pattern
												// is disable.
			} else {
				ServiceReference ref = getServiceReference();
			...
			if (m_type == 0) { // Array
				try {
					if (refs == null) {
						...
						// exception. This method allows setting the component
						// type.
						Object[] objs = (Object[]) Array.newInstance(getSpecification(),
								refs.length);
						for (int i = 0; refs != null && i < refs.length; i++) {
							ServiceReference ref = refs[i];
					...
					m_handler
							.error("Cannot create the array - Check that the bundle can access the service interface",
									e);
		...
		if (m_usage != null) {
			...
			if (usage.m_stack > 0) {
				if (usage.dec()) {
					...
					usage.clear();
					m_usage.set(usage); // Set the Thread local as value has
										// been modified
				}
			}
		}
	...
	private class SmartProxyFactory extends ClassLoader {
		...
		public Object getProxy(Class spec, Dependency dep) {
			try {
				...
				Constructor constructor = clazz.getConstructor(new Class[] { clazz
						.getClassLoader().loadClass(Dependency.class.getName()) });
		...
		public Class loadClass(String name) throws ClassNotFoundException {
			try {
				return getHandler().getInstanceManager().getContext().getBundle()
						.loadClass(name);
			} catch (ClassNotFoundException e) {
	...
	private class DynamicProxyFactory implements InvocationHandler {
		...
		public Object getProxy(Class spec) {
			return java.lang.reflect.Proxy.newProxyInstance(getHandler()
