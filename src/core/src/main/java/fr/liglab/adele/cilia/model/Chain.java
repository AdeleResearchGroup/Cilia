...
public class Chain extends Component {
	 ...
	 */
	private Hashtable /*<Mediator>*/ mediators = new Hashtable();
	 ...
	 */
	public Chain(String id, String type, String nspace, Dictionary properties) {
	...
	public boolean add(Mediator mediator) {
		...
		synchronized (lockObject) {
			if (mediator != null && !mediators.containsKey(mediatorId) && (!adapters.containsKey(mediatorId))) {
				...
				throw new RuntimeException("Mediator identifier must be unique: " + mediator.getId());
			}
		}
		if (inserted) {
	...
	public boolean removeMediator(String mediatorId) {
		...
		synchronized (lockObject) {
			mediatorToRemove = (Mediator) mediators.remove(mediatorId);
	...
	public boolean add(Adapter adapter) {
		...
		synchronized (lockObject) {
			if (adapter != null && !adapters.containsKey(adapter.getId()) && (!mediators.containsKey(adapter.getId()))) {
				...
				throw new RuntimeException("Adapter identifier must be unique: " + adapter.getId());
			}
		}
		if (inserted) {
	...
	public boolean removeAdapter(String adapterId) {
		...
		//Remove Mediators and bindings from the chain.
		synchronized (lockObject) {
			adapterToRemove = (Adapter) adapters.remove(adapterId);
	 ...
	 * @param sourcePort Mediator's delivery port.
	 * @param targetPort Mediator's entry port.
	 * @return The Binding which associes two mediators, null when it can't associate them.
	 */
	...
	public Binding bind(Port sourcePort, Port targetPort, Binding binding) {
		...
		if (null != binding.getSourcePort() || null != binding.getTargetPort()) {
			//System.err.println("Binding is assigned to other components");
			return null;
		}
		...
		if (!sourcePort.getType().equals(PortType.OUTPUT)) { 
			throw new RuntimeException("Port type not compatible, it must be PortType.OUTPUT");
		}
		if (!targetPort.getType().equals(PortType.INPUT)) { 
			throw new RuntimeException("Port type not compatible, it must be PortType.INPUT");
		}
		//set the ports.
	...
	public Binding[] getBindings(MediatorComponent source, MediatorComponent target) {
		...
		if (sourceB != null && targetB != null) {
			...
			allBindings = (Binding[])result.toArray(new Binding[result.size()]);
		//} else {
